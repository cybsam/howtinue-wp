jQuery(document).ready(function($) {
	...
	$( document ).on( 'click', '.ephd-wp__widget-form .ephd-wp__found-locations-list li', function( e ) {
		e.stopPropagation();
		if ( $( this ).find( '.ephd-wp__location-assigns' ).length > 0 ) {
			return false;
		}
	...
	// Load form to edit/create Widget
	$( document ).on ( 'click', '.ephd-admin__widget-preview .ephd_edit_widget, .ephd-wp__create-new-widget-btn', function( e ) {
		e.preventDefault();
		...
		ephd_send_ajax( postData, function( response ){
			...
			if ( ! response.error && typeof response.message != 'undefined' ) {
				...
				// Auto save new added widget as draft
				setTimeout( function() {
	...
	$( document ).on( 'click', '.ephd-wp__widget-form .ephd-wp__widget-action__publish-btn, .ephd-wp__widget-form .ephd-wp__widget-action__draft-btn', function( e ) {
		...
		ephd_send_ajax( postData, function( response ){
			if ( ! response.error && typeof response.message != 'undefined' ) {
				...
				if ( $( '.ephd-admin__widget-preview--' + postData.widget_id ).length ) {
					// Update preview box for Widget
					$( '.ephd-admin__widget-preview--' + postData.widget_id ).replaceWith( response.widget_preview );
				} else {
	...
	// Update demo preview when any select option was changed on FAQs, Search tab
	$( document ).on( 'change', '.ephd-wp__widget-form .ephd-admin__form-tab-wrap--faqs select, .ephd-wp__widget-form .ephd-admin__form-tab-wrap--search select', function() {
		if ( $( this ).val() === 'new' ) {
	...
	// Update demo preview when any checkbox was clicked on FAQs tab
	$( document ).on( 'click', '.ephd-wp__widget-form .ephd-admin__form-tab-wrap--faqs input[type="checkbox"]', function() {
		update_preview( this );
	} );
	...
	// Update demo preview when any text was changed on Labels tab
	$( document ).on( 'change', '.ephd-wp__widget-form .ephd-admin__form-tab-wrap--labels input[type="text"]', function() {
		update_preview( this );
	} );
	...
	// Update demo preview when any checkbox was clicked on Dialog tab
	$( document ).on( 'change', '.ephd-wp__widget-form .ephd-admin__form-tab-wrap--global-settings input', function() {
		update_preview( this );
	} );
	...
	// Update demo preview when any radio button was clicked on Appearance tab -> Colors -> Predefined Colors
	$( document ).on( 'click', '.ephd-wp__widget-form .ephd-wp__options-container--predefined-colors input[type="radio"]', function() {
		update_preview( this );
	} );
	...
	// Update demo preview when any input was changed on Global Settings tab (exception: Widget Name)
	/* not required because preview on global-settings page is disabled
	$( document ).on( 'change', '.ephd-wp__widget-form .ephd-wp__options-container--global-settings input[type="text"]:not([name="widget_name"])', function() {
		update_preview( this );
	});
	$( document ).on( 'change', '.ephd-wp__widget-form .ephd-wp__options-container--global-settings input[type="number"]', function() {
		update_preview( this );
	});
	$( document ).on( 'change', '.ephd-wp__widget-form .ephd-wp__options-container--global-settings select', function() {
		update_preview( this );
	});
	*/
	...
	function get_widget_form_data( form, action ) {
		...
		return {
			...
			// Appearance
			design_id: form.find( '[name="design_id"]' ).val(),
			...
			dialog_display_mode: form.find( '[name="dialog_display_mode"]:checked' ).val(),
			...
			contact_title_header: form.find( '[name="contact_title_header"]' ).val(),
			contact_title: form.find( '[name="contact_title"]' ).val(),
