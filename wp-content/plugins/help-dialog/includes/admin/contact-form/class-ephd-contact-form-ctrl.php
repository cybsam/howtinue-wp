...
class EPHD_Contact_Form_Ctrl {
	...
	public function delete_all_submissions() {
		...
		wp_die( json_encode( array( 'status' => 'success', 'message' => esc_html__( 'All submissions removed.', 'help-dialog') ) ) );
	}
	...
	public function save_contact_form() {
		...
		if ( is_wp_error( $contact_forms_config ) ) {
			EPHD_Utilities::ajax_show_error_die( EPHD_Utilities::report_generic_error( 214 ) );
		}
		...
		if ( is_wp_error( $updated_contact_forms_config ) ) {
			EPHD_Logging::add_log( 'Error occurred on saving Contact Forms configuration. (217)' );
			EPHD_Utilities::ajax_show_error_die( EPHD_Utilities::report_generic_error( 217, $updated_contact_forms_config ) );
		}
		$updated_contact_form = $updated_contact_forms_config[$contact_form['contact_form_id']];
	...
	public function delete_contact_form() {
		...
		$contact_forms_config = ephd_get_instance()->contact_forms_config_obj->get_config( true );
		if ( is_wp_error( $contact_forms_config ) ) {
			EPHD_Logging::add_log( 'Failed to retrieve Contact Forms configuration (220)' );
			EPHD_Utilities::ajax_show_error_die( EPHD_Utilities::report_generic_error( 220, $contact_forms_config ) );
		}
		...
		$updated_contact_form_config = ephd_get_instance()->contact_forms_config_obj->update_config( $contact_forms_config );
		if ( is_wp_error( $updated_contact_form_config ) ) {
			EPHD_Logging::add_log( 'Error occurred on saving Contact Form configuration. (222)' );
			EPHD_Utilities::ajax_show_error_die( EPHD_Utilities::report_generic_error( 222, $updated_contact_form_config ) );
		}
	...
	public function load_contact_form() {
		...
		$contact_forms_config = ephd_get_instance()->contact_forms_config_obj->get_config( true );
		if ( is_wp_error( $contact_forms_config ) ) {
			EPHD_Logging::add_log( 'Failed to retrieve Contact Forms configuration (226)' );
	...
	private static function get_sanitized_contact_form_from_input() {
		...
		$contact_form['contact_title'] = EPHD_Utilities::post( 'contact_title' );
		if ( empty( $contact_form['contact_title'] ) ) {
	...
	public function load_contact_form_preview() {
		...
		$contact_forms_config = ephd_get_instance()->contact_forms_config_obj->get_config( true );
		if ( is_wp_error( $contact_forms_config ) ) {
			EPHD_Logging::add_log( 'Failed to retrieve Contact Forms configuration (240)' );
			EPHD_Utilities::ajax_show_error_die( EPHD_Utilities::report_generic_error( 240, $contact_forms_config ) );
		}
