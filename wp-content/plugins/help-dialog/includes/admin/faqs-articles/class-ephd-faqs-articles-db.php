...
/**
 * CRUD for Help Dialog FAQs Articles data
 *
 * @property string primary_key
 * @property string table_name
 */
class EPHD_FAQs_Articles_DB extends EPHD_DB {
	...
	public function insert_faq( $id, $faq_id, $question, $answer, $status, $lang=false ) {
		...
		if ( empty( $id ) ) {
			return new WP_Error( 'db-insert-error', 'Could not insert Help Dialog FAQ Article record' );
		}
		...
		if ( empty( $faq_id ) ) {
			...
			if ( empty( $update_result ) ) {
				return new WP_Error( 'db-update-error', 'Could not update Help Dialog FAQ Article record: ' . $id );
			}
		}

		$faq = parent::get_by_primary_key( $id );
		if ( empty( $faq ) || is_wp_error( $faq ) ) {
			return is_wp_error( $faq ) ? $faq: new WP_Error( 'db-insert-error', 'Could not insert Help Dialog FAQ Article record.' );
		}
	...
	/**
	 * Delete FAQ Article by faq_id (all language)
	 *
	 * @param $faq_id
	 * @return bool
	 */
	...
	private static function get_allowed_statuses() {
		...
		if ( self::can_user_read_private_articles() ) {
	...
	/**
	 * Is current user roles included for reading private FAQ articles
	 *
	 * @return bool
	 */
	private static function can_user_read_private_articles() {
