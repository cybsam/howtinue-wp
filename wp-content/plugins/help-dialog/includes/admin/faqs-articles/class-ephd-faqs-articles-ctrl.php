...
/**
 * Handle user submission from Help dialog FAQs/Articles
 */
class EPHD_FAQs_Articles_Ctrl {
	...
	public function save_question_data() {
		...
		foreach ( $question_languages as $lang ) {
			...
			$answer = stripslashes( EPHD_Utilities::post( 'question_content_' . $lang, '', 'wp_editor' ) );
	...
	public function delete_question() {
		...
		$faqs_page_handler = new EPHD_FAQs_Articles_Page( $widgets_config );
		...
		foreach ( $widgets_config as $widget_config ) {
			...
			$faqs_preview_config = array(
				...
				'html'          => EPHD_HTML_Admin::get_item_preview_box( $widget_config, array(
					...
					'icon_html'             => EPHD_FAQs_Articles_Page::get_faqs_icon_html(),
	...
	public function save_faqs() {
		...
		// retrieve sequence of article ids from request
	...
	public function update_faqs_preview() {
		...
		$hd_handler = new EPHD_Help_Dialog_View( $widget, $designs_config[$widget['design_id']], true, true );
		wp_die( json_encode( array(
			'status'        => 'success',
			'message'       => 'success',
