...
class EPHD_Analytics_DB extends EPHD_DB {
	 ...
	 * @return bool|int
	 */
	public function count_event( $widget_id, $page_id, $object_id, $column_name, $event_meta, $value = 1 ) {

		// Check if event param and column value exist
		if ( ! isset( $event_meta['type_id'] ) || ! isset( $event_meta['aggregation'] ) || empty( $column_name ) ) {
			EPHD_Logging::add_log( 'Analytics update: Could not retrieve Event Meta fields' );
			return false;
		}
