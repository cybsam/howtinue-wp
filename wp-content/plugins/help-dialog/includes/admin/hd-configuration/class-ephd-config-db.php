...
class EPHD_Config_DB {
	...
	const EPHD_DESIGNS_CONFIG_NAME = 'ephd_designs_config';
	const EPHD_CONTACT_FORMS_CONFIG_NAME = 'ephd_contact_forms_config';
	const EPHD_NOTIFICATION_RULES_CONFIG_NAME = 'ephd_notification_rules_config';

	private $option_name;
	private $cached_settings = null;
	private $is_global_options;
	private $default_config;
	private $fields_specification;

	public function __construct( $option_name ) {
		$this->option_name = $option_name;
		$this->is_global_options = $this->option_name == self::EPHD_GLOBAL_CONFIG_NAME;
	...
	public function get_config( $return_error=false ) {
		...
		if ( is_array( $this->cached_settings ) ) {
			...
			$config_array = $this->is_global_options ? [ $this->cached_settings ] : $this->cached_settings;
			foreach ( $config_array as $instance_id => $cached_setting ) {
				$config[$instance_id] = wp_parse_args( $cached_setting, $this->default_config );
			}

			return $this->is_global_options ? $config[0] : $config;
		}

		// retrieve Plugin config
		$config = get_option( $this->option_name, [] );

		if ( empty( $config ) || ! is_array( $config ) || ( ! $this->is_global_options && empty( $config[EPHD_Config_Specs::DEFAULT_ID] ) ) ) {
	...
	public function update_config( $config ) {
		...
		// only Global Config cannot be empty, the rest can be empty array
		if ( ! is_array( $config ) || ( empty( $config ) && $this->option_name == self::EPHD_GLOBAL_CONFIG_NAME ) ) {
		...
		$sanitized_config = $this->is_global_options ? $sanitized_config[0] : $sanitized_config;
