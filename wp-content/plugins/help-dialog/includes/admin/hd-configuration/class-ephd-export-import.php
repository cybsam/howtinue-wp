...
class EPHD_Export_Import {
	...
	public function import_hd_config() {
		...
		if ( ! is_array( $imported_configs ) ) {
			$this->message['error'] = __( 'Import failed due to invalid data.', 'help-dialog' );
			return $this->message;
		}

		// check if we need to upgrade data
		$this->upgrade_plugin_data();
	...
	private function get_all_hd_configs() {
		...
		$configs[EPHD_Config_DB::EPHD_CONTACT_FORMS_CONFIG_NAME] = ephd_get_instance()->contact_forms_config_obj->get_config();
		$configs[EPHD_Config_DB::EPHD_DESIGNS_CONFIG_NAME] = ephd_get_instance()->designs_config_obj->get_config();
	...
	private function update_all_hd_configs( $new_configs ) {
		...
		$contact_forms_config = isset( $new_configs[ EPHD_Config_DB::EPHD_CONTACT_FORMS_CONFIG_NAME] ) ? $new_configs[ EPHD_Config_DB::EPHD_CONTACT_FORMS_CONFIG_NAME] : $current_configs[ EPHD_Config_DB::EPHD_CONTACT_FORMS_CONFIG_NAME];
		$updated_config = ephd_get_instance()->contact_forms_config_obj->update_config( $contact_forms_config );
		if ( is_wp_error( $updated_config ) ) {
			$this->message['error'] =  'E32 ' . $updated_config->get_error_message();  // do not translate
			return;
		}

		// update Design config
		$designs_config = isset( $new_configs[ EPHD_Config_DB::EPHD_DESIGNS_CONFIG_NAME] ) ? $new_configs[ EPHD_Config_DB::EPHD_DESIGNS_CONFIG_NAME] : $current_configs[ EPHD_Config_DB::EPHD_DESIGNS_CONFIG_NAME];
		$updated_config = ephd_get_instance()->designs_config_obj->update_config( $designs_config );
		if ( is_wp_error( $updated_config ) ) {
			$this->message['error'] =  'E33 ' . $updated_config->get_error_message();  // do not translate
			return;
		}
	...
	private function upgrade_plugin_data() {
		$import_plugin_version = empty( $plugin_config['plugin_version'] ) ? '' : $plugin_config['plugin_version'];
		$last_version = empty( $import_plugin_version ) ? '1.0.0' : $import_plugin_version;
		if ( $last_version != Echo_Help_Dialog::$version ) {
			EPHD_Upgrades::run_upgrade( $last_version );
		}
	}
