...
/**
 * CRUD for Help Dialog Channels data
 *
 * @property string primary_key
 * @property string table_name
 */
class EPHD_Channels_DB extends EPHD_DB {

	const PRIMARY_KEY = 'channel_id';
	const DEFAULT_ID = 1;
	const CONFIG_KEY = 'config';
	const EPHD_CHANNELS_CONFIG_NAME = 'ephd_channels_config';
	...
	public function __construct() {
		...
		$this->table_name = $wpdb->prefix . 'ephd_channels';
		$this->primary_key = self::PRIMARY_KEY;

		$this->config_spec = EPHD_Config_Specs::get_fields_specification( self::EPHD_CHANNELS_CONFIG_NAME );
		$this->default_config = EPHD_Config_Specs::get_default_hd_config( self::EPHD_CHANNELS_CONFIG_NAME );
	}
	...
	public function get_column_defaults() {
		return array(
			'channel_id'           => 0, // use 0 here as need to insert a new record, while in specs we have default 1
	...
	public function update_config( $widgets_config ) {
		...
		foreach ( $widgets_config as $channel_id => $widget_config ) {
			...
			$updated_widgets_config[$channel_id] = wp_parse_args( $new_widget_config, $this->default_config );

			// convert back to config array
			foreach ( $this->config_spec as $spec_name => $spec ) {
				if ( ! in_array( $spec_name, self::get_column_names() ) ) {
					$updated_widgets_config[ $channel_id ][ self::CONFIG_KEY ][ $spec_name ] = $updated_widgets_config[ $channel_id ][ $spec_name ];
					unset( $updated_widgets_config[ $channel_id ][ $spec_name ] );
				}
			}

			$updated_widgets_config[$channel_id][self::CONFIG_KEY]['channel_id'] = $channel_id;
			$record = self::serialize_values( $updated_widgets_config[$channel_id] );
			if ( empty( $record ) ) {
				EPHD_Logging::add_log( "Can't serialize Widget record. Widget ID: " . $channel_id );
				return new WP_Error( 'update_widget_config', "Can't serialize Widget record. Widget ID: " . $channel_id );
			}

			$result = parent::upsert_record( $channel_id, $record );
			if ( empty( $result ) ) {
				EPHD_Logging::add_log( "Can't Insert/Update Widget record. Widget ID: " . $channel_id );
				return new WP_Error( 'update_widget_config', "Can't Insert/Update Widget record. Widget ID: " . $channel_id );
			}

			// merge configs into a one-dimensional array
			$updated_widgets_config[$channel_id] = array_merge( $updated_widgets_config[$channel_id], $updated_widgets_config[$channel_id][self::CONFIG_KEY] );
			unset( $updated_widgets_config[$channel_id][self::CONFIG_KEY] );
		}
	...
	public function get_config( $return_error=false ) {
		...
		if ( is_array( $this->cached_settings ) ) {
			$widgets_config = [];
			foreach ( $this->cached_settings as $channel_id => $cached_setting ) {
				$widgets_config[$channel_id] = wp_parse_args( $cached_setting, $this->default_config );
			}
		...
		if ( is_wp_error( $widget_config_records ) || empty( $widget_config_records ) || ! is_array( $widget_config_records ) ) {
		...
		foreach ( $widget_config_records as $widget_config_obj ) {
			...
			// use defaults for missing configuration
			$channel_id = empty( $widget_config['channel_id'] ) ? self::DEFAULT_ID : $widget_config['channel_id'];
			$widgets_config[$channel_id] = wp_parse_args( $widget_config, $this->default_config );
		}
	 ...
	 * @return bool
	 */
	public function delete_widget_by_id( $channel_id ) {
		...
		$deleted = parent::delete_record( $channel_id );
		if ( empty( $deleted ) ) {
			EPHD_Logging::add_log( "Can't delete Widget records. Widget ID: " . $channel_id );
			return false;
		}
	 ...
	 * @return array | false
	 */
	public function get_widget_config_by_id( $channel_id ) {
		...
		$widget_record = $this->get_by_primary_key( $channel_id );
