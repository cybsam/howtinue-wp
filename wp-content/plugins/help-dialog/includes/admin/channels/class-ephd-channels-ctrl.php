...
*/
	...
	public function create_widget() {
		...
		$global_config = self::get_updated_global_config_from_input( $global_config );
	...
	/**
	 * Update WIDGET preview when user changes settings - nothing is saving here
	 */
	public function update_preview() {
		...
		// retrieve and apply Design changes - do not save, preview only
		$designs_config[$widget['design_id']] = self::get_updated_design_from_input( $designs_config );
		...
		$hd_handler = new EPHD_Help_Dialog_View( $widget, $designs_config[$widget['design_id']], $is_opened, true );
		wp_die( json_encode( array(
			'status'        => 'success',
			'message'       => 'success',
	...
	private static function get_sanitized_contact_forms_from_input( $contact_forms_config ) {
		...
		$contact_forms_config['contact_acceptance_text'] = EPHD_Utilities::post( 'contact_acceptance_text' );
		$contact_forms_config['contact_button_title'] = EPHD_Utilities::post( 'contact_button_title' );
		$contact_forms_config['contact_success_message'] = EPHD_Utilities::post( 'contact_success_message' );
	 ...
	 * @return array|null
	 */
	private static function get_updated_global_config_from_input( $global_config ) {
		...
		*/
		...
		$global_config['initial_message_id'] = $initial_message_text != $global_config['initial_message_text'] || $initial_message_image_url != $global_config['initial_message_image_url']
			? current_time( 'timestamp', true )
			: $initial_message_id;

		// retrieve Show/Hide Initial Message
		$global_config['initial_message_toggle'] = EPHD_Utilities::post( 'initial_message_toggle' );

		// sign Message Text
		$global_config['initial_message_text'] = $initial_message_text;

		// sign Message Image URL
		$global_config['initial_message_image_url'] = EPHD_Utilities::post( 'initial_message_image_url' );
