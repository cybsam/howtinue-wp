...
class EPHD_Utilities {
	...
	public static function ajax_verify_nonce_and_admin_permission_or_error_die() {
		...
		// check wpnonce
		$wpnonce_value = self::post( '_wpnonce_ephd_ajax_action' );
		if ( empty( $wpnonce_value ) || ! wp_verify_nonce( $wpnonce_value, '_wpnonce_ephd_ajax_action' ) ) {
			self::ajax_show_error_die( __( 'Login or refresh the page to edit the Help Dialog.', 'help-dialog' ) );
		}
	...
	public static function ajax_show_info_die( $message, $title='', $type='success' ) {
		if ( defined('DOING_AJAX') ) {
			wp_die( json_encode( array( 'message' => EPHD_HTML_Forms::notification_box_bottom( $message, $title, $type ) ) ) );
		}
	}
	...
	public static function ajax_show_error_die( $message, $title = '', $error_code = '' ) {
		if ( defined('DOING_AJAX') ) {
			wp_die( json_encode( array( 'error' => true, 'message' => EPHD_HTML_Forms::notification_box_bottom( $message, $title, 'error' ), 'error_code' => $error_code ) ) );
		}
	}
