...
function ephd_activate_plugin_do() {
	...
	$handler->create_table();
	$handler = new EPHD_FAQs_Articles_DB();
	$handler->create_table();
	...
	if ( empty( $plugin_version ) ) {
		...
		// prepare Designs config
		$config = ephd_get_instance()->designs_config_obj->get_config();
		ephd_get_instance()->designs_config_obj->update_config( $config );

		// prepare Contact Forms config
		$config = ephd_get_instance()->contact_forms_config_obj->get_config();
		ephd_get_instance()->contact_forms_config_obj->update_config( $config );
	...
	// Clear permalinks
	update_option( 'ephd_flush_rewrite_rules', true );
