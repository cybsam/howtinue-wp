...
class EPHD_Search {
	...
	public static function search( $is_front=true ) {
		...
		$design = empty( $widgets_config[$widget_id] ) ? $designs_config[EPHD_Config_Specs::DEFAULT_ID] : $designs_config[$widgets_config[$widget_id]['design_id']];
		...
		$no_results_sc = '<div class="ephd-hd-no-results__img"><img alt="' . esc_attr__( 'No FAQs Defined', 'help-dialog' ) . '" src="' . esc_url( Echo_Help_Dialog::$plugin_url . 'img/no-faqs-defined.jpeg' ) . '" /></div>' .
						'<div class="ephd-hd__search_results__no-results-container"> ' .
							'<div class="ephd-hd-no-results__msg">' . esc_html( $design['no_results_found_title_text'] ) . '</div>' .
							'<div class="ephd-hd-no-results__keywords"></div>' .
							'<div class="ephd-hd-no-results__hints">' .
		                        wp_kses_post( wp_unslash( $design['no_results_found_content_html'] ) ) .
							'</div>' .
						'</div>' .
						'<div class="ephd-hd-no-results__contact-us">' .
							'<span class="ephd-hd__contact-us__link" data-ephd-target-tab="contact">' . wp_kses_post(  wp_unslash( $design['no_result_contact_us_text'] ) ) . '</span>' .
						'<div>';

		// require minimum size of search word(s)
		if ( empty( $search_terms ) ) {
			wp_die( json_encode( array( 'status' => 'success', 'no_results' => $no_results_sc ) ) );
		}
		...
		$widget = $widgets_config_handler->get_widget_config_by_id( $widget_id );
		if ( empty( $widget ) ) {
			wp_die( json_encode( array( 'status' => 'success', 'no_results' => $no_results_sc ) ) );
		}
	...
	public static function get_post_content() {
		...
		if ( ! EPHD_Utilities::is_positive_int( $post_id ) ) {
			wp_die( json_encode( array( 'status' => 'error', 'post_content' => '' ) ) );
		}
		...
		if ( $post_status == 'protected' ) {
			$post_content = $design['protected_article_placeholder_text'];
		} else {
	...
	private static function search_in_faqs( $widget, $search_terms ) {
		...
		foreach ( $faq_results as $faq ) {
			$faq_search_result[] = EPHD_FAQs_Articles_Page::get_faq_item_html( $faq->id, $faq->question, $faq->answer );
		}
	...
	private static function search_in_kbs( $widget, $search_terms ) {
		...
		$article_results = array_merge( $article_results, $kb_search_results );
