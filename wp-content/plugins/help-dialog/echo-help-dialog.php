 ...
 * Description: Show the Help Dialog on any pages, so your customers can browse the FAQs and knowledge base articles right from the chat window or contact you without leaving the page.
 * Version: 1.21.0
 * Author: Echo Plugins
...
if ( ! defined( 'EPHD_PLUGIN_NAME' ) ) {
	define( 'EPHD_PLUGIN_NAME', 'Help Dialog and FAQs' );
}
...
final class Echo_Help_Dialog {
	...
	/* @var EPHD_Config_DB */
	public $designs_config_obj;
	/* @var EPHD_Config_DB */
	public $contact_forms_config_obj;
	/* @var EPHD_Config_DB */
	...
	public static function instance() {
		...
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Echo_Help_Dialog ) ) {
	...
	private function setup_system() {
		...
		self::$instance->designs_config_obj = new EPHD_Config_DB( EPHD_Config_DB::EPHD_DESIGNS_CONFIG_NAME );
		self::$instance->contact_forms_config_obj = new EPHD_Config_DB( EPHD_Config_DB::EPHD_CONTACT_FORMS_CONFIG_NAME );
	...
	private function setup_plugin() {
		...
		if ( is_admin() || ( defined( 'WP_CLI' ) && WP_CLI ) ) {	// || ( defined( 'REST_REQUEST' ) && REST_REQUEST )
            if ( $this->is_hd_plugin_active_for_network( 'help-dialog/echo-help-dialog.php' ) ) {
	...
	public function setup_backend_classes() {
		...
		if ( EPHD_Core_Utilities::is_help_dialog_admin_page( $request_page ) ) {
			...
			switch ( $request_page ) {
				...
				// Channels page
				case 'ephd-help-dialog-channels':
					add_action( 'admin_enqueue_scripts', 'ephd_load_admin_help_dialog_channels_script' );
					break;

				// FAQs/Articles page
				case 'ephd-help-dialog-faqs-articles':
    ...
    private function is_hd_plugin_active_for_network( $plugin ) {
        if ( ! is_multisite() ) {
            return false;
        }
